...
namespace Admin.NETApp.Core.Service
{
        ...
        where TImportDto : class, new()
    {
        ...
        public virtual async Task<FileContentResult> Export(TSearchDto searchDto)
        {
            ...
            // 导出文件
            var byteArray = await exporter.ExportAsByteArray(entities.Adapt<List<TExportDto>>());
        ...
        private void GetSearchParameters(TSearchDto searchDto)
        {
            ...
            foreach (var keyValuePair in searchDictionary)
            {
                ...
                if (!entityPropertyNames.Contains(keyValuePair.Key) || keyValuePair.Value == null) continue;
